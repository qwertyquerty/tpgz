...
void onCreate() {
    g_menuMgr->setPersistentData(new AmountsData);
...
void onLoad() {
    l_menu = new AmountsMenu(*g_menuMgr->getPermanentData<Cursor>(),
                             *g_menuMgr->getPersistentData<AmountsData>());
...
void onDelete() {
    AmountsData* data = g_menuMgr->getPersistentData<AmountsData>();
    delete data;
