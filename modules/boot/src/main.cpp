...
void main() {
    ...
    if (l_gzIconTex.loadCode == TEX_UNLOADED) {
...
KEEP_FUNC void GZ_handleSavingTmp() {
    ...
    if (last_frame_was_loading && !l_fopScnRq_IsUsingOfOverlap) {
...
#ifdef GCN_PLATFORM
        mDoCPd_c::getCpadInfo(0).mPressedButtonFlags = mDoCPd_c::getCpadInfo(0).mButtonFlags;
#endif

#ifdef WII_PLATFORM
        if (!g_menuMgr->isOpen()) {
            GZ_getWiiPadStatus().trig = GZ_getWiiPadStatus().hold;
        }
#endif
    }
}
    ...
    if (g_menuMgr->isOpen()) {
        ...
        if (l_gzIconTex.loadCode == TEX_OK) {
            Draw::drawRect(0xFFFFFFFF, makeVec2(spriteOffset.x, 5.0f),
                           makeVec2(30 * (isWidescreen ? 0.75f : 1.0f), 30), &l_gzIconTex._texObj);
        }
    }
}
...
KEEP_FUNC void GZ_renderPlayPause() {
    if (GZStng_getData(STNG_TOOLS_FRAME_ADVANCE, false)) {
        if (l_framePauseTex.loadCode == TEX_UNLOADED) {
            load_texture("/tpgz/tex/framePause.tex", &l_framePauseTex);
        }

        if (l_framePlayTex.loadCode == TEX_UNLOADED) {
